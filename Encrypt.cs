...
namespace NitroHelper
{
  public static class Encrypt
  {
    ...
    public static void EncryptArm9(char[] gameCode, ref byte[] secureArea)
    {
      ...
      init1(cardheader_gamecode);
      encrypt(card_hash, ref d, ref c);
      BitConverter.GetBytes(c).CopyTo(secureArea, p * 4);
      BitConverter.GetBytes(d).CopyTo(secureArea, p * 4 + 4);
    }

    static uint[] card_hash = new uint[0x412];
    static uint[] arg2 = new uint[3];

    private static void init1(uint cardheader_gamecode)
    {
    ...
    private static void init2(ref uint[] magic, uint[] a)
    {
      encrypt(magic, ref a[2], ref a[1]);
      encrypt(magic, ref a[1], ref a[0]);
      update_hashtable(ref magic, new byte[] {
        (byte)(a[0] & 0xFF), (byte)((a[0] >> 8) & 0xFF), (byte)((a[0] >> 16) & 0xFF), (byte)((a[0] >> 24) & 0xFF),
        (byte)(a[1] & 0xFF), (byte)((a[1] >> 8) & 0xFF), (byte)((a[1] >> 16) & 0xFF), (byte)((a[1] >> 24) & 0xFF),
        (byte)(a[2] & 0xFF), (byte)((a[2] >> 8) & 0xFF), (byte)((a[2] >> 16) & 0xFF), (byte)((a[2] >> 24) & 0xFF),
      });
    }

    private static void encrypt(uint[] magic, ref uint arg1, ref uint arg2)
    {
      uint a, b, c;
      a = arg1;
      b = arg2;
    ...
    private static uint lookup(uint[] magic, uint v)
    {
      uint a = (v >> 24) & 0xFF;
      uint b = (v >> 16) & 0xFF;
      uint c = (v >> 8) & 0xFF;
    ...
    private static void update_hashtable(ref uint[] magic, byte[] arg1)
    {
      for (int j = 0; j < 18; j++)
      {
        uint r3 = 0;
